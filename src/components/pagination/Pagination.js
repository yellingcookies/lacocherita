...
const Pagination = ({currentPage, setCurrentPage, productsPerPage, totalProducts}) => {
    ...
    return <ul className={styles.pagination}>
        <li onClick={paginatePrev} className={currentPage === pageNumbers[0] ? `${styles.hidden}` : null}>Ant</li>
        ...
        <li onClick={paginateNext} className={currentPage === pageNumbers[pageNumbers.length - 1] ? `${styles.hidden}` : null}>Sig</li>
        <p>
            <b className={styles.page}>{`Página ${currentPage}`}</b>
            <span>{` de `}</span>
